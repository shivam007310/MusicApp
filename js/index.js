...
async function getSongs(folder) {
  ...
  let a = await fetch(`http://192.168.137.1:5500/${folder}/`);
...
async function displayAlbums() {
  let a = await fetch(`http://192.168.137.1:5500/songs/`);
  // console.log(await a.text());
  let response = await a.text();
  let div = document.createElement("div");
  div.innerHTML = response;
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    if (e.href.includes("/songs/")) {
      ...
      let a = await fetch(
        `http://192.168.137.1:5500/songs/${folder}/info.json`
      );
      console.log(a);
...
async function main() {
  ...
  document.querySelector(".mute").addEventListener("click", (e) => {
    ...
    //in if block e.target.src=="volume.svg"..this will not work because value of e.targer.src is "http://192.168.137.1:5500/img/volume.svg" and not "voume.svg"
